...
async function loadCredentials() {
  try {
    ...
    // ?ê³—ë’ª??ê¹?????ê¼?ë¨?½Œ???´Ñ? ?ëº¤ì½‰??PKCE???ê³•ëœ‘??°ë£„ client_secret???¾ëŒ?œå«„?åª›ìˆˆ??è¹‚ë?ê¶????¸ë•²??
...
async function refreshCalendarEventTitle(todo) {
  ...
  const cleanTitle = cleanGoogleText(todo.text).replace(/^\[?„ë£Œ\]\s*/, '');
  const summary = todo.status === 'completed' ? '[?„ë£Œ] ' + cleanTitle : cleanTitle;
  ...
  for (const todo of localTodos) {
    ...
    const taskBody = {
      ...
      try {
        ...
        // ìº˜ë¦°???´ë²¤???½ìž…/?…ë°?´íŠ¸ ?œê±° (?¬ìš©???”ì²­: êµ¬ê? ?œìŠ¤?¬ë§Œ ?¤ì–´ê°€ê³??¼ì •?€ ?¤ì–´ê°€ì§€ ?Šë„ë¡?
      } catch (err) {
