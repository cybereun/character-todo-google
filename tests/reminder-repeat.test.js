...
test('side panels preserve the base window anchor and character position', () => {
  ...
  assert.match(styles, /\.widget\[data-calendar-mode="true"\] \.character-button \{[\s\S]*?left: 134px;/);
  assert.match(styles, /\.widget\[data-search-mode="true"\] \.character-button \{[\s\S]*?left: 134px;/);
  assert.match(styles, /\.widget\[data-schedule-mode="true"\] \.character-button \{[\s\S]*?left: 134px;/);
});
...
test('schedule ribbon is available with colorful styling', () => {
  ...
  assert.match(styles, /\.speech-bubble \{[\s\S]*?bottom: 132px;[\s\S]*?left: 38px;[\s\S]*?z-index: 12;/);
