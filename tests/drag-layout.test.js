...
test('character and speech bubble share one drag anchor', () => {
  ...
  assert.match(styles, /\.character-anchor \.speech-bubble\s*\{[\s\S]*?left:\s*-96px;[\s\S]*?bottom:\s*124px;/);
...
test('side panels use fixed positions beside the todo panel', () => {
  ...
  assert.match(styles, /\.widget\[data-calendar-mode="true"\]\s+\.calendar-panel\s*\{[\s\S]*?left:\s*412px;[\s\S]*?right:\s*auto;/);
  assert.match(styles, /\.widget\[data-search-mode="true"\]\s+\.search-panel\s*\{[\s\S]*?left:\s*396px;[\s\S]*?right:\s*auto;/);
  assert.match(styles, /\.widget\[data-schedule-mode="true"\]\s+\.schedule-panel\s*\{[\s\S]*?left:\s*396px;[\s\S]*?right:\s*auto;/);
});
